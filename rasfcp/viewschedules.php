...
tr:nth-child(even) {
    ...
    <a href="studentportal.php"> <button type="button" class="btn btn-primary" aria-label="Left Align">
  <span class="glyphicon glyphicon-home" aria-hidden="true"></span>Home</button></a><br/><br>
    
<?php
    session_start();
    include ('conn1.php');
    $queryf=mysqli_query($conn,"select course from schedules where userid='".$_SESSION['id']."'") or die(mysqli_error());
while($coz=mysqli_fetch_array($queryf)){
    $course=$coz['course'];
}
...
if ($result = $mysqli->query("SELECT * FROM schedules where course='".$course."'"))
{
